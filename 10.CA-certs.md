...
- Poner toda la configuración en el directorio `input`
...
- Generar la información requerida en el directorio `output`.
...
La configuración está en el fichero `input\rootCAconfig.bat`:
- Datos del sujeto de la CA raíz de confianza.
- Nombre de los ficheros de su certificado y clave privada.

Ejecutar desde el directorio `bin` el script `2_selSignedRootCA.bat` y seguir los pasos en pantalla.
...
La configuración está en el fichero `input\serverSideCertificate.bat`.
...
Ejecutar desde el directorio `bin` el script `31_serverSideCertificate.bat` y seguir los pasos en pantalla.
...
Ejecutar desde el directorio `bin` el script `32_importServerCertKeystore.bat` y seguir los pasos en pantalla.
...
La configuración está en el fichero `input\userCertificate.bat`, y también `input\rootCAconfig.bat`, que sería otro distinto si los certificados fuesen de diferentes CA.
...
Ejecutar desde el directorio `bin` el script `51_userCAtruststore.bat` y seguir los pasos en pantalla.
...
Ejecutando `53_clientSideCertificate.bat`
